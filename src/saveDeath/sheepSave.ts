...
import { Specialization_onSave, Specialization_onSpawn } from "abilities/sheep/specialization";
import { bloodlust, replaceUnit } from "./common";
...
export const onSheepSave = (savedUnit: unit, savingUnit: unit): void => {
  ...
  DisplayTextToPlayer(
    ...
    0,
    0,
    `${colorizedName(savedPlayer)} has been ${colorize.sheepblue("saved")} by ${colorizedName(savingPlayer)}!`,
  );
