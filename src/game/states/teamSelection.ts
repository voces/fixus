...
import { forEachPlayer, mapEachPlayer, timeout } from "util/temp";
import { fetch } from "util/networkio";
import { getTeams } from "./helpers/getTeams";
import { isSandbox, sheepTeam, TriggerRegisterPlayerEventAll, wolfTeam } from "shared";
import { reloadMultiboard } from "util/multiboard";
import { gameState, transitionGame, TransitionInformation, transitionsFrom } from "./common";
...
const finalizeTeams = (): void => {
  [...preferences.entries()].forEach(([player, preference]) => logPreference(player, preference.preference));
