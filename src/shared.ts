...
export const getterSetterFunc = <T>(init?: T): ((newValue?: T) => T) => {
  ...
  return (newValue?: T): T => {
    ...
    if (value === undefined) throw new Error("variable left undefined");
...
export const gameState: (
  newState?: GAME_STATES
) => GAME_STATES = getterSetterFunc("init" as GAME_STATES);
export const goldFactor: (newFactory?: number) => number = getterSetterFunc(1);
export const isHere = (): boolean =>
...
const countHereRealEnum = (): void => {
  if (
    ...
    GetPlayerController(GetEnumPlayer()) === MAP_CONTROL_USER
  )
    someInteger = someInteger + 1;
};
...
export const TriggerRegisterPlayerChatEventAll = (
  t: trigger,
  s: string,
  match: boolean
): void => {
...
export const TriggerRegisterPlayerEventAll = (
  t: trigger,
  e: playerevent
): void => {
  let i = 0;
...
export const TriggerRegisterPlayerUnitEventAll = (
  t: trigger,
  p: playerunitevent,
  b: boolexpr
): void => {
...
export const mainUnit = (p: player): unit => {
  ...
  if (IsPlayerInForce(p, wolfTeam)) return wolves[playerId];

  if (IsPlayerInForce(p, sheepTeam)) return sheeps[playerId];
...
export const wolfUnit = (p: player): unit => {
  if (!IsPlayerInForce(p, wolfTeam))
    throw new Error("used wolfUnit on non-wolf");

  const playerId = GetPlayerId(p);
  const wolf = wolves[playerId];

  if (GetUnitTypeId(wolf) === WHITE_WOLF_TYPE) return wws[playerId];
...
export const SmallText = (
  ...
  if (
    GetUnitAbilityLevel(u, FourCC("Alv1")) <= 0 &&
    IsVisibleToPlayer(GetUnitX(u), GetUnitY(u), GetLocalPlayer())
  ) {
