...
import { onConstructionStart, onSpellCast } from "util/event";
...
const specializations: Record<string, SpecializationData> = {
  ...
  engineer: {
    ...
    passive: FourCC("A009"),
    active: FourCC("A00G"),
    customFarmLogic: (farm: unit, level: number): void => BlzSetUnitMaxHP(farm, BlzGetUnitMaxHP(farm) + 10 * level),
  },
  attacker: {
    learn: FourCC("A00E"),
    passive: FourCC("A00K"),
...
export const playerSpecializations: PlayerSpecialization[] = fillArrayFn(
  bj_MAX_PLAYERS,
  () => ({ level: 0, maxLevel: 0, specialization: null }),
);
    ...
    timeout("sheep specialization unused spellbok", 30, () => {
      if (u && UnitAlive(u) && BlzGetUnitAbility(u, SPELLBOOK_ABILITY_TYPE) != null) {
        DisplayTextToPlayer(
          ...
          0,
          0,
          "You've saved someone! You can specialize by clicking the spellbook icon.",
        );
      }
    });
