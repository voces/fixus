...
import { forEachPlayer, forEachPlayerUnit, timeout, withSelectedUnits } from "util/temp";
import { fillArray } from "shared";
import { onCreated, onDeath, onSpellCast } from "util/event";
...
const onNextShop = (): void => {
  ...
  switch (type) {
    case BLUE_SHOP:
      return SelectUnit(data.green, true);
    case GREEN_SHOP:
      return SelectUnit(data.red, true);
    case RED_SHOP:
      return SelectUnit(data.blue, true);
    default:
      throw "Bad switch";
  }
};
...
export const addQuickShop = (unit: unit): void => {
  ...
  const green = CreateUnit(
    neutralPassive,
    GREEN_SHOP,
    x + Math.cos(angle + offset) * 128,
    y + Math.sin(angle + offset) * 128,
    270,
  );
  const red = CreateUnit(
    neutralPassive,
    RED_SHOP,
    x + Math.cos(angle + offset * 2) * 128,
    y + Math.sin(angle + offset * 2) * 128,
    270,
  );
