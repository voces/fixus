...
import { applyCloaksOnMirrorImages } from "wolves/specializations/index";
...
const countCloaks = (u: unit): number => {
  ...
  for (let i = 0; i < bj_MAX_INVENTORY; i++)
    if (GetItemTypeId(UnitItemInSlot(u, i)) === CLOAK_TYPE) cloaks++;
...
const onDropItem = (): void => {
  if (
    GetItemTypeId(GetManipulatedItem()) === CLOAK_TYPE &&
    countCloaks(GetManipulatingUnit()) === 1
  )
          ...
          withTempGroup(unitsToDamage => {
            ...
            while (damagedUnit = FirstOfGroup(unitsToDamage)) {
              if (
                ...
                const damage =
                  IsUnitIllusion(cloakHolder) ||
                  !applyCloaksOnMirrorImages(GetOwningPlayer(cloakHolder))
                    ? 6 - cloaks
                    : 15 - cloaks * 2;
