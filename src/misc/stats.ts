import { defineEvent, defineNumberValue, defineStringValue, emitCustom, setPlayerFlag } from "w3ts-w3mmd";
import { addScriptHook, W3TS_HOOK } from "@voces/w3ts";
import { fillArray, isSandbox, isUnitSheep, isUnitWolf, saveskills, WISP_TYPE, wolfTeam, wolfUnit } from "shared";
...
import { onConstructionStart, onDeath } from "util/event";
import { gameState, getRemainingTime } from "game/states/common";
import { BUILD, REPO, VERSION } from "./buildInfo";
 ...
 */
export const onWolfGoldBonus = (gold: number, reset?: boolean, remainingOverride?: number): number => {
  if (reset) {
    currentWolfGoldFactor = 1;
    return 1;
  }

  const remaining = remainingOverride ?? (gameState() === "play" ? getRemainingTime() : 25 * 60);
