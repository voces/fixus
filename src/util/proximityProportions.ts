...
import { isComputer, isPlayingPlayer } from "./player";
 ...
 */
const sigmoid = (v: number, b: number, c: number): (t: number) => number => {
  const cTetrated = c ** c;
  const k = (cTetrated + 1) ** v;
  return (t: number): number => k / (cTetrated + Math.exp(b * t)) ** v;
};
...
const wolfProximityProportions = (
  ...
  forEachPlayer((p) => {
    ...
    const proportion = reducePlayerUnits(p, (max, unit) => {
      ...
      const distance = (xDelta ** 2 + yDelta ** 2) ** 0.5;
      const proportion = (killType === "sheep" ? wolfSheepKillSigmoid : wolfOtherKillSigmoid)(distance) *
        unitFactor(unit);
