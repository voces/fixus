import { parse, stringify, Value } from "./json";
import { addScriptHook, File, W3TS_HOOK } from "@voces/w3ts";
 ...
 */
const calcInterval = (tries: number): number => 60 * (tries + 1.3 ** tries) / 1000;
...
const checkForResponse = (request: Request): void => {
  ...
  const response = rawResponse == null ? null : prefixed ? rawResponse.slice(1) : rawResponse;
...
addScriptHook(W3TS_HOOK.MAIN_AFTER, (): void => {
  ...
  let outstanding = 0;
  timeout("networkio fetch proxy", 0, () =>
    forEachPlayer((p) => {
